...
func play_sound(sound_path: String, bus: String = "SFX", volume_percent: float = 100.0) -> AudioStreamPlayer:
...
func play_music(stream: AudioStream, loop: bool = true, fade_duration: float = 1.0):
	# If same music is already playing at normal volume, do nothing
	if current_music_player and current_music_player.stream == stream and current_music_player.playing and current_music_player.volume_db >= -0.1 and not is_fading:
		return
	...
	if is_fading and is_fading_out and current_music_player and current_music_player.stream == stream:
		...
		fade_tween.tween_property(current_music_player, "volume_db", AudioServer.get_bus_volume_db(AudioServer.get_bus_index("Music")), fade_duration * 0.5)
	...
	if current_music_player and current_music_player.playing and (current_music_player.stream != stream or current_music_player.volume_db <= -0.1):
		is_fading = true
		is_fading_out = true
		fade_tween = create_tween()
		...
		fade_tween.tween_callback(func(): 
			...
			if current_music_player and current_music_player.stream == stream:
				return
			_create_new_music_player(stream, fade_duration)
		)
	else:
...
func _create_new_music_player(stream: AudioStream, fade_duration: float):
	# Create new player
	...
	current_music_player.volume_db = -80.0 if fade_duration > 0 else 0.0
	...
	if fade_duration > 0:
		is_fading = true
		fade_tween = create_tween()
		fade_tween.tween_property(current_music_player, "volume_db", 0.0, fade_duration)
